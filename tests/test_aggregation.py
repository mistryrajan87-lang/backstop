...
def test_per_token_attribution():
    ...
    check("no derived band, verdict or shape label is emitted",
          not ({"shape", "verdict", "band"} & set(o)), str(sorted(o)))
...
def test_degenerate():
    ...
    check("an empty run emits no label either",
          not ({"shape", "verdict", "band"} & set(snap["overall"])),
          str(sorted(snap["overall"])))
    ...
    check("an even vector is described by its numbers, not by a label",
          not ({"shape", "verdict", "band"} & set(b2)), str(sorted(b2)))
